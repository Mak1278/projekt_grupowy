...
<form name="form" action="account.php?q=1" method="post"/>

<select name="Wyb_kat">
  <option value="Zadna" selected>Wybierz kategorie</option>
  <option value="Zabawa">Zabawa</option>
  <option value="F">kat.2</option>
</select>
...
<tr><td style="vertical-align:middle"><b>Rank</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Username</b></td><td style="vertical-align:middle"><b>Score</b></td></tr>';
	...
	$total=100;
    if($total >= $showfrom){
        $q = mysqli_query($con, "SELECT * FROM rank ORDER BY score DESC, time ASC LIMIT ".($showfrom-1).",100") or die('Error223');
        while ($row = mysqli_fetch_array($q)) {
            $e = $row['username'];
            $s = $row['score'];
